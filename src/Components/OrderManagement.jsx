...
const OrderManagement = () => {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
  if (searchTerm) setFilterDate("all");
}, [searchTerm]);
  ...
  const setupSSEConnection = () => {
    ...
    if (vendorData._id) {
      ...
      console.error(
        "❌ localStorage vendor_user:",
        localStorage.getItem("vendor_user")
      );
    }

    // Listen for connection status
    const removeConnectedListener = notificationService.addListener(
      "connected",
      () => {
    ...
    const removeDisconnectedListener = notificationService.addListener(
      "disconnected",
      () => {
    ...
    const removeNewOrderListener = notificationService.addListener(
      "new_order",
      (order) => {
    ...
    const removeOrderUpdateListener = notificationService.addListener(
      "order_updated",
      (order) => {
    ...
    try {
      ...
      const transformedOrders = ordersData.map((order) => ({
        ...
        delivery: {
          ...
          deliveryBoyPhone:
            order.delivery?.deliveryBoy?.phoneNumber ||
            order.delivery?.deliveryBoy?.phone,
  ...
  const fetchAvailableDeliveryBoys = async () => {
    try {
      ...
      if (response && response.success && response.data) {
        ...
        const available = allTeam.filter(
          (db) => db.deliveryBoyInfo?.isAvailable !== false && db.isActive
        );
...
const matchesSearch =
  ...
  order.customer?.phone?.includes(search);
    ...
    const matchesStatus =
      filterStatus === "all" ||
      (filterStatus === "assigned" && order.delivery?.deliveryBoy) ||
...
// 🔑 IMPORTANT: disable date filtering when searching
if (!searchTerm && filterDate !== "all") {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            >
              <WifiIcon
                className={`h-3.5 w-3.5 ${sseConnected ? "animate-pulse" : ""}`}
              />
          ...
          <p className="mt-0.5 text-xs text-gray-500">
            ...
            {sseConnected && "• Real-time updates enabled"}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-6">
        ...
        <div
          onClick={() =>
            setFilterStatus(filterStatus === "confirmed" ? "all" : "confirmed")
          }
          className={`bg-white overflow-hidden rounded-lg border-2 cursor-pointer transition-all ${
        ...
        <div
          onClick={() =>
            setFilterStatus(
              filterStatus === "processing" ? "all" : "processing"
            )
          }
          className={`bg-white overflow-hidden rounded-lg border-2 cursor-pointer transition-all ${
        ...
        <div
          onClick={() =>
            setFilterStatus(
              filterStatus === "unassigned" ? "all" : "unassigned"
            )
          }
          className={`bg-white overflow-hidden rounded-lg border-2 cursor-pointer transition-all ${
            filterStatus === "unassigned"
                  ...
                  <tr key={order.id} className="hover:bg-gray-50">
                      ...
                      {order.delivery?.deliveryBoy ? (
                        <div className="flex flex-col gap-1">
                          ...
                          <span className="text-xs text-gray-500">
                            (
                            {order.delivery.partner === "lalaji_network"
                              ? "Lalaji Network"
                              : "Self Delivery"}
                            )
                          </span>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        {vendorProfile?.vendorInfo?.deliveryModel ===
                          "self_delivery" &&
                          !order.delivery?.deliveryBoy &&
                          ["packed", "processing", "confirmed"].includes(
                            order.status
                          ) && (
                            <button
                              onClick={async () => {
                    ...
                    >
                      <div className="flex items-center flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500">
                            {item.sku && `SKU: ${item.sku} • `}₹{item.price} ×{" "}
                            {item.quantity}
                          </div>
                        </div>
                      </div>
                          ...
                          <p className="text-xs text-green-800 mt-1">
                            ...
                            {selectedOrder.status === "packed" && (
                              <span className="block mt-1 font-medium">
                                📦 Order is ready for pickup. Delivery partner
                                will update status from here.
                              </span>
                            )}
                          </p>
                        </div>
                    ...
                    <div className="mt-2 p-3 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-start">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-blue-800 mt-1">
                            The delivery partner is managing this order. Status
                            updates will be handled by the delivery team.
                          </p>
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <div className="text-xs text-gray-500 mt-1">
                              Delivery Model:{" "}
                              {vendor.deliveryModel === "self_delivery" ? (
              ...
              <div className="bg-white px-6 py-4 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      Select a delivery person from your team for Order #
                ...
                {allDeliveryTeam.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-gray-600 mb-2">
                      No delivery persons in your team
                    </p>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between mb-2">
                      <label className="block text-sm font-medium text-gray-700">
                        Select Delivery Person ({allDeliveryTeam.length} team
                        members)
                      </label>
                    ...
                    <div className="space-y-2">
                      {allDeliveryTeam.map((deliveryBoy) => {
                        ...
                        const isCurrentlyAssigned =
                          deliveryBoy.deliveryBoyInfo?.currentOrderId;
                        ...
                        return (
                          <div
                            ...
                            onClick={() =>
                              canSelect &&
                              setSelectedDeliveryBoy(deliveryBoy._id)
                            }
                    ...
                    <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <p className="text-xs text-blue-900 flex items-center">
                        ...
                        {availableDeliveryBoys.length} of{" "}
                        {allDeliveryTeam.length} delivery persons are available
                        for assignment
                      </p>
                    </div>
                  </div>
